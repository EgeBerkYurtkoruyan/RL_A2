...
class Trainer_buffer_target:
    ...
    def train_qmodel(self):
        ...
        while total_steps < max_steps:
            ...
            while not terminal_val and total_steps < max_steps:
                ...
                if len(self.memory) >= buffer_batch_size and total_steps % update_ratio == 0:
                    ...
                    with torch.no_grad():
                        max_next_q = torch.max(target_net(next_states), dim=1)[0]
        ...
        return avg_rewards, steps_list
    def train_repetitions(self, num_iterations: int=1):
        rewards_reps = []
        steps_reps = []
        episodes_reps = []
        print(f"Training model over {num_iterations} repetitions with Experience Replay and Target network")
        
        for it in range(num_iterations):
            print(f"Running iteration: {it+1}")
            avg_rewards, steps_list = self.train_qmodel()
