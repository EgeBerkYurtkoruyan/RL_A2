...
class Trainer_target_network:
    ...
    def train_qmodel(self,
        ...
        target_network_update = target_network_update if target_network_update is not None else self.config["training"]["target_network_update"]



        # Set epsilon to starting value
        if adaptive_epsilon:
        ...
        # Initialize target network with the same weights as policy network
        ...
        print(f"Target network update frequency: {target_network_update}")
        ...
        while total_steps < max_steps:
            ...
            while not terminal_val and total_steps < max_steps:
                ...
                if total_steps % update_ratio == 0:
                    with torch.no_grad():
                        # Use target network to calculate the target Q-value
                        target = reward + gamma * torch.max(target_net(next_state)) * (1-terminal_val)
                ...
                episode_steps += 1
                pbar.update(1)
                if total_steps >= next_reward_print:
                    current_avg_reward = np.mean(rewards_list[-avg_window:]) if len(rewards_list) >= avg_window else np.mean(rewards_list) if rewards_list else 0
                    print(f"\nStep {total_steps}: Average reward over last {min(avg_window, len(rewards_list))} episodes: {current_avg_reward:.2f}")
                    next_reward_print += 10000
...
if __name__ == "__main__":
    ...
    exp_name = "dqn_with_target_network_exp_1" 

    dqn_trainer = Trainer(env_name, Net=Qnet, config_file=config_file, results_path=results_path)
    
    # Train model over n repetitions
    rewards_reps, steps_reps, episodes_reps = dqn_trainer.train_repetitions(num_iterations=iterations)
